...
## 400 错误：unknown variant 'model'

**现象**：使用 DeepSeek 和夏瑾预设时报错 `400`，详细内容包含 `…unknown variant 'model'…`

**原因**：夏瑾较新版本使用 SPreset 内置 NoAss，而用户多开了错误配置的 NoAss 脚本/扩展，导致请求体格式异常。

**解决**：排查并关闭冲突的扩展，尤其注意：
- **酒馆助手**中的"压缩[相邻/系统]消息"脚本
- **JS Runner** 扩展
- **NoAss** 扩展
